...
if button:
    ...
    hrr=mhr-int(rhr)
    hrr_rhr=hrr+int(rhr)
    ut2=[round(0.65*hrr+int(rhr),2),round(0.75*hrr+int(rhr),2)]
    ut1=[round(0.75*hrr+int(rhr),2),round(0.8*hrr+int(rhr),2)]
    at=[round(0.8*hrr+int(rhr),2),round(0.85*hrr+int(rhr),2)]
    an=[round(0.85*hrr+int(rhr),2), "Your Max"]
    result={"Type":["Min","Max"],
            "UT2":ut2,
            "UT1":ut1,
            "AT":at,
